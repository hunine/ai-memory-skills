---
description: Load Obsidian vault context for a project or project alias (index, daily note, needs-review, recent sessions)
argument-hint: <project-folder-name-or-alias>
---
...
2. Match `$ARGUMENTS` case-insensitively against `<vault>/Projects/*` directory names, then against aliases declared in project `index.md` frontmatter as `aliases` or `alias`.
3. On no match, list available projects and aliases, then stop. On ambiguous alias, list matching projects and stop.
